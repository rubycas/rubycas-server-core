...
module RubyCAS::Server::Core
  module Tickets
    ...
    # Generates one time login ticket for given client
    ...
    class << self
      %w{login_ticket service_ticket ticket_granting_ticket}.each do |ticket_type|
        ...
        # ticket_valid? method
        #
        # Passing a ticket string to one of these methods will find it 
        # (delegated to the persistence module) and ask it if it's valid.
    ...
    # Generates a ServiceTicket for the supplied service and links it to
    # the supplied TicketGrantingTicket
